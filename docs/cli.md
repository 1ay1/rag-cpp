...
| `--source` | Chunk **code on definition boundaries** instead of prose windows. Uses the hand-written chunker for known languages and **infers the conventions** of any language it does not know — an in-house DSL, a vendor format, anything. Measured on CodeSearchNet (4005 real queries): **MRR@10 0.8193 vs 0.7507** and **Recall@1 0.7328 vs 0.6517** for the language-blind window fallback, in a **34% smaller index** ([BENCHMARKS.md](../BENCHMARKS.md#does-integrity-translate-to-retrieval)). Safe on a mixed corpus: inference declines on prose. See [Formats and languages](formats.md). |
